...
정답
- a (객체 자신을 가리키는 참조변수는 예를 들어
Student s = new Student(); 에서 s를 말한다.)
```
```agsl
...
정답
c (매개변수 타입은 상관 있어도 리턴 타입은 상관 없다)
d (매개변수 이름은 상관 없다.)
```
```agsl
...
정답
- c (생성자가 가장 나중이다)
- b (지역변수는 초기화가 필요 없고 매개변수 또한 지역변수이다.)
- e (클래스 변수가 먼저다)
```
```agsl
...
2. str에 "ABC123" 저장.
3. 저장된 str print실행
4. change 메서드(매개변수str(ABC123)) 실행.
5. 매개변수 ABC123에 문자열 456이 붙는다
6. print로 ABC123456 출력 
```