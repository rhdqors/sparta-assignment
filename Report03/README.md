...
4-1. 다음의 문장들을 조건식으로 표현해보세요.
        int형 변수 x가 10 보다 크고 20 보다 작을 때 true인 조건식
        int x = 0;
        if (x > 10 && x < 20) {
        ...
        int형 변수 year가 400 으로 나눠떨어지거나 또는 4 로 나눠떨어지고 100 으로 나눠떨어지지 않을때 true인 조건식
        int year = 0;
...
4-4번은 현재 풀고있는 문제이다.
처음에는 for 문으로 접근했었고 두번째는 절대값으로 접근하여 1부터 100까지 더하는 방식을 해보기도 했다.
하지만 문제 의도는 for, while 등의 반복문을 쓰고 기호 구분도 따로 해야할 것 같아 다시 풀어보고 있다.
